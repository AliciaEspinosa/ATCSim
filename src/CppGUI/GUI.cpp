...
GUI::render() {
    ...
    textDisplay->displayText("ATCSim", 15, 25, win_width, win_height, ORANGE);
...
GUI::DrawFlight(ATCDisplay::ATCDFlight flight) {
    ...
    if (flight.focused) {
        ...
        textDisplay->displayText(const_cast<char *>(flight.id.c_str()), 15, 75, GUI::win_width, GUI::win_height, LIGHTBLUE, GLUT_BITMAP_HELVETICA_18);
        ...
        if (!route.empty()) {
            ...
            for (it = route.begin(); it != route.end(); ++it) {
                ...
                if (it->name != "") {
                    ...
                    glRasterPos2i((*it).x + 1.5*wpt_size, (*it).y - 0.5*wpt_name.length()*0.5*wpt_size);    // TODO(fmrico): center text
            ...
            textDisplay->displayText(const_cast<char *>("Route"), 15, 230, GUI::win_width, GUI::win_height, BLUE, GLUT_BITMAP_HELVETICA_12);
...
>>>>>>> 52bb8dda4cecb806e11a8f5fa300e2ce47bd86c4
}
